...
- [ ] 2. Commit all in-progress changes to main
...
- [ ] 3. Replace the root `Dockerfile` with the correct Go multi-stage build
...
- [ ] 4. Verify `docker-compose.yml` is correct
...
- [ ] 5. Verify the existing `docker.yml` CI workflow satisfies Docker publish requirements
...
- [ ] 6. Create `.github/workflows/release.yml` for cross-platform binary releases
...
- [ ] 7. Verify version injection and `go.mod` module path
